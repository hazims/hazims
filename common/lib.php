...
function pms_recursive_rmdir($dirname, $contentOnly = false, $followLinks = false)
{
    if (is_dir($dirname) && !is_link($dirname)) {
        ...
        while ($iterator->valid()) {
            if (!$iterator->isDot()) {
                if ($iterator->isLink() && false === (bool) $followLinks) $iterator->next();
...
function pms_get_coords($address)
{
    ...
    if (isset($lat) && isset($lng)) return array($lat, $lng);
    else return false;
}
/***********************************************************************
...
{
    if (
        isset($_SESSION[$name . '_token']) && isset($_SESSION[$name . '_token_time'])
